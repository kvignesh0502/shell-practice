...
if [ $USERID -eq 0 ]
then
    echo -e "User is root...$G Proceed $N" | tee -a $LOG_FILE
else
    echo -e "$R ERROR : Run with Root $N " | tee -a $LOG_FILE
    exit 1
fi
...
if [ $? -eq 0 ]
then
    echo -e "Nothing to do...$Y mysql installed $N" | tee -a $LOG_FILE
else
    echo "NOT installed.. Installing Now" | tee -a $LOG_FILE
    dnf install mysql -y
    VALIDATE $? mysql
fi
...
if [ $? -eq 0 ]
then
    echo -e "Nothing to do... $Y Nginx installed $N" | tee -a $LOG_FILE
else
    echo "NOT installed.. Nginx Installing Now" | tee -a $LOG_FILE
    dnf install nginx -y
    VALIDATE $? Nginx
fi
